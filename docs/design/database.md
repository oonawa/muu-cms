...
spaces                      ハコ
space_users                 ハコとユーザーの中間テーブル（ロール管理）
blueprints                  モノ
...
|--------|-----|------|
| id | BIGINT UNSIGNED PK | |
| space_id | BIGINT UNSIGNED FK | → spaces.id |
| user_id | BIGINT UNSIGNED FK | → users.id |
| role | ENUM('admin') | MVPはadminのみ。UNIQUE(space_id, user_id) |
| created_at | TIMESTAMP | |
...
| id | BIGINT UNSIGNED PK | |
| space_id | BIGINT UNSIGNED FK | → spaces.id |
| slug | VARCHAR(64) | 半角英数スラッグ。UNIQUE(space_id, slug) |
...
|--------|-----|------|
| id | BIGINT UNSIGNED PK | |
| blueprint_id | BIGINT UNSIGNED FK UNIQUE | → blueprints.id（1モノ1スペック） |
| created_at | TIMESTAMP | |
...
|--------|-----|------|
| id | BIGINT UNSIGNED PK | |
| blueprint_id | BIGINT UNSIGNED FK | → blueprints.id |
...
users ──< space_users >── spaces
spaces ──< blueprints
blueprints ──── specs ──< parameters ──── parameter_constraints (0 or 1)
blueprints ──< contents
```
